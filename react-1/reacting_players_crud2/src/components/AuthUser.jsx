 ...
 */
>>>>>>> 00f272a903dabc6a502196c0bbbb4c572853ee73
...
export const AuthUser = ({ isLoggedIn, onLogin, onRegister, onLogout }) => {
  ...
  const handleSubmit = (event) => {
    ...
    const username = event.target.elements['auth-username'].value;
    const password = event.target.elements['auth-password'].value;
  ...
  return (
    <div>
      <a href="#" onClick={handleLinkClick}>
        {isLoggedIn ? 'Logout' : view === 'login' ? 'Go to register' : 'Go to login'}
      </a>
      {!isLoggedIn && (
        <form id="auth-form" onSubmit={handleSubmit}>
          ...
          <input
            ...
            required
          />
          <button id="submit-player" className="btn-auth" type="submit">
